...
{
    public class HGSBase_Yataimeshi : UdonSharpBehaviour
    {
        [Header("This")]
        ...
        [Header("PassDown")]
        [Tooltip("ピックアップ系と同時に子にも何かさせたい時用　ビール等で利用")]
        [SerializeField] bool PassDownPickupState;

        [Header("Other")]
        [SerializeField] Transform defaultTr;
        [SerializeField][Tooltip("ピックアップ時にオーナー書き換え処理で使用、セット必須")] GameObject[] hGSCoreGoArray;
        ...
        {
            if (!Utilities.IsValid(Networking.LocalPlayer)) { SendCustomEventDelayedFrames(nameof(OnPickup), 1); return; }

            ///State3へ移行///
            SendCustomNetworkEvent(NetworkEventTarget.All, nameof(State3_PickedUpByHand));

            foreach (GameObject go in hGSCoreGoArray)
            {
                ///待機状態の子オブジェクト達のオーナーシップを書き換え///
                if (!Utilities.IsValid(go)) { continue; }
                UdonBehaviour _udonBehaviour = go.GetComponent<UdonBehaviour>();
                if ((int)_udonBehaviour.GetProgramVariable("foodState") == 1)
                {
                    Networking.SetOwner(Networking.LocalPlayer, go);
                }

                ///PassDownオンなら、子のOnPickupInhも発火///
                if (PassDownPickupState)
                {
                    _udonBehaviour.SendCustomEvent("OnPickupInh");
                }
            }
        ...
        {
            ///子のOnPickupUseDownInhも発火///
            if (!PassDownPickupState) { return; }
            foreach (GameObject go in hGSCoreGoArray)
            {
                if (!Utilities.IsValid(go)) { continue; }
                go.GetComponent<UdonBehaviour>().SendCustomEvent("OnPickupUseDownInh");
            }
        }

        public override void OnPickupUseUp()
        {
            ///子のOnPickupUseUpInhも発火///
            if (!PassDownPickupState) { return; }
            foreach (GameObject go in hGSCoreGoArray)
            {
                if (!Utilities.IsValid(go)) { continue; }
                go.GetComponent<UdonBehaviour>().SendCustomEvent("OnPickupUseUpInh");
            }
        }
        ...
        public override void OnDrop()
        {
            ...
            {
                if (!Utilities.IsValid(go)) { continue; }
                go.GetComponent<UdonBehaviour>().SendCustomEvent("OnDropInh");
            }
        }
